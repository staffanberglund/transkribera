...
use self::{
    loop_ui::{
        active_loop_bounds, connect_waveform_loop_gesture, rebuild_loop_list,
        repeat_active_loop_if_needed,
    },
    marker_ui::{MarkerDirection, connect_marker_controls, jump_to_marker, rebuild_marker_list},
    navigation::{
        seek_relative, seek_to_position, set_playback_anchor, update_position, update_time_label,
    },
...
fn connect_seek_scale(state: &Rc<UiState>) {
    let weak = Rc::downgrade(state);
    state
        .seek
